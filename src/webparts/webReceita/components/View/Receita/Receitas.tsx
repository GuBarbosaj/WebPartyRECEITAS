...
import { IViewReceitasProps } from './IViewReceitasProps';
import { IGeneralProps } from '../../../../../shared/services/interfaces/iGeneralProps';
...
const ViewReceitas: React.FunctionComponent<IViewReceitasProps> = (props) => {
    ...
    async function getReceitas():Promise<IReceitas[]> {
      ...
      if(props.receitaIdList){
        resultReceitas = await spList.
        getList(props.receitaIdList,['ID','Receita', 'TipoReceita', 'Cara', 'DataTentativa'],
    );
    ...
    function transformaGeneralParaReceitas(general: IGeneralProps): IReceitas[] {
      ...
      general.forEach((item:IGeneralProps) => {
        receitas.push({
          ...
          Cara: item.Cara,
          DataTentativaString: "",
          DataTentativa: item.DataTentativa,
          Anexo: ""
        });
      })
